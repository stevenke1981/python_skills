...
class PluginFixture:
    def __init__(self, root: Path) -> None:
        ...
        shutil.copytree(skill, root / "skills/py-example")
        (root / ".codex-plugin").mkdir(parents=True)
        (root / ".codex-plugin/plugin.json").write_text(json.dumps({
            "name": "python-engineering-skills", "version": "1.0.0",
            "description": "Test plugin", "skills": "./skills/"}), encoding="utf-8")
        (root / ".agents/plugins").mkdir(parents=True)
        (root / ".agents/plugins/marketplace.json").write_text(json.dumps({
            "name": "test", "plugins": [{"name": "python-engineering-skills",
                "source": {"source": "local", "path": "./"},
                "policy": {"installation": "AVAILABLE", "authentication": "ON_INSTALL"},
                "category": "Developer Tools"}]}), encoding="utf-8")
        (root / "skills-manifest.json").write_text(json.dumps({
            "skills": [{"name": "py-example"}]}), encoding="utf-8")
        (root / "scripts").mkdir()
        for name in ("install_codex.py", "uninstall_codex.py", "install_codex.sh",
                     "install_codex.ps1", "codex_fs.py"):
            (root / "scripts" / name).write_text("# test\n", encoding="utf-8")
...
class InstallerTests(unittest.TestCase):
    def test_plugin_install_preserves_other_marketplace_entries_and_replaces_old_copy(self) -> None:
        with tempfile.TemporaryDirectory() as source_temp, tempfile.TemporaryDirectory() as home_temp:
            source, home = Path(source_temp), Path(home_temp)
            PluginFixture(source)
            marketplace = home / ".agents/plugins/marketplace.json"
            marketplace.parent.mkdir(parents=True)
            marketplace.write_text(json.dumps({"name": "existing", "plugins": [
                {"name": "other-plugin", "source": {"source": "local", "path": "./other-plugin"}}]}), encoding="utf-8")
            old = home / ".codex/plugins/python-engineering-skills"
            old.mkdir(parents=True)
            (old / "old.txt").write_text("old", encoding="utf-8")
            # Legacy copies have no ownership receipt: overwriting is now explicit.
            result = INSTALLER.main(["--mode", "plugin", "--scope", "user", "--force",
            ...
            self.assertEqual({item["name"] for item in payload["plugins"]},
                             {"other-plugin", "python-engineering-skills"})

    def test_uninstall_preserves_unrelated_plugin(self) -> None:
        with tempfile.TemporaryDirectory() as source_temp, tempfile.TemporaryDirectory() as home_temp:
            source, home = Path(source_temp), Path(home_temp)
            PluginFixture(source)
            args = ["--mode", "plugin", "--scope", "user", "--home", str(home),
                    "--source-root", str(source)]
            self.assertEqual(INSTALLER.main(args), 0)
            marketplace = home / ".agents/plugins/marketplace.json"
            payload = json.loads(marketplace.read_text(encoding="utf-8"))
            payload["plugins"].append({"name": "other-plugin",
                                     "source": {"source": "local", "path": "./other-plugin"}})
            marketplace.write_text(json.dumps(payload), encoding="utf-8")
            self.assertEqual(UNINSTALLER.main(args), 0)
